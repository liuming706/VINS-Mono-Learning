 ...
 * @return  vector<std::pair<vector<ImuConstPtr>, PointCloudConstPtr>> (IMUs,
 * img_msg)s
 */
...
void feature_callback(const sensor_msgs::PointCloudConstPtr &feature_msg) {
  ...
  m_buf.unlock();
  // 在这种情况下，notify_one 通知一个可能在等待 con
  // 的线程（比如一个消费者线程）数据已经被放入了 feature_buf
  // 中，允许它进行相应的处理
  con.notify_one();
}
...
void process() {
  while (true) {
    std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>,
                          sensor_msgs::PointCloudConstPtr>>
        measurements;
    //  lk 对象构造时自动锁定 m_buf 互斥锁。
    std::unique_lock<std::mutex> lk(m_buf);
    // 等待数据imu与feature接收完成就会被唤醒
    // 在提取measurements时互斥锁 m_buf 会锁住，此时无法接收数据
    // 而在等待con被唤醒的期间，lk释放持有的互斥锁，使其他线程可以获得锁并修改共享数据
    con.wait(lk,
             [&] { return (measurements = getMeasurements()).size() != 0; });
    // 手动解锁 m_buf 互斥锁。这个操作在 std::unique_lock
    // 析构时通常会自动发生，但在这里它显式地被调用，用于释放锁，这样后续的操作就可以继续进行
    lk.unlock();
    ...
    for (auto &measurement : measurements) {
      ...
      //建立每个特征点的(camera_id,[x,y,z,u,v,vx,vy])的map，索引为feature_id
      ...
      pubKeyframe(
          estimator); //"keyframe_point"、"keyframe_pose" 关键帧位姿和点云
