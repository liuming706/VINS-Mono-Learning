 ...
 *              goodFeaturesToTrack()
 * 添加特征点(shi-tomasi角点)，确保每帧都有足够的特征点
 *              addPoints()添加新的追踪点
 *              undistortedPoints()
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time) {
  ...
  if (EQUALIZE) {
    //自适应直方图均衡
    // OpenCV 库中的 CLAHE（Contrast Limited Adaptive Histogram
    // Equalization）功能来提高图像的对比度 3.0：这是 CLAHE
    // 对象的对比度限制参数（clip
    // limit），较高的值会使得对比度更高，但可能会引入噪声。 cv::Size(8,
    // 8)：这是用于直方图均衡的矩形区域的大小，
  ...
  if (PUB_THIS_FRAME) {
    ...
    if (n_max_cnt > 0) {
       ...
       *指示是否使用Harris角点检测，如不指定则使用shi-tomasi算法 double  k =
       *0.04                Harris角点检测需要的k值
       *)
       */
  ...
  //根据不同的相机模型去畸变矫正和转换到归一化坐标系上，计算较角点速度
  undistortedPoints();
  prev_time = cur_time;
}
